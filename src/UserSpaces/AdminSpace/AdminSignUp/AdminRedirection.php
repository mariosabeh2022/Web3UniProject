...
if(!empty($_POST['FirstName'])&&
   ...
   !empty($_POST['Password'])){
    ...
    if($row=$statement->fetch(PDO::FETCH_ASSOC)){
        echo "<script>alert('You have entered an existing admin');</script>";
    ...
    $query_user="INSERT INTO users (FirstName,MiddleName,LastName,Username,Email,Password,Role) VALUES
                ('$FN','$MN','$LN','$User','$Email','$Password','Admin')";
    ...
    $query="INSERT INTO admin (IDA,FirstName,MiddleName,LastName,Username,Email,Password)
            VALUES ('$ID','$FN','$MN','$LN','$User','$Email','$Password');";
    $ex = $PDO->prepare($query);
    $ex->execute();
    $query="INSERT INTO creates (IDA,CreatesIDA) VALUES ('$ID','$IDA');";
    $ex = $PDO->prepare($query);
    $ex->execute();
...
header("refresh:3, url=AdminSignUp.php");
?>