    ...
    <div class="container">
      ...
      <div class="collapse navbar-collapse" id="ftco-nav">
        <ul class="navbar-nav ml-auto">
          ...
          <li class="nav-item"><a href="../Export/StudentExportToExcel.php" class="nav-link">Export to excel sheet</a></li>
        </ul>
      </div>
    </div>
  </nav>
  <nav class="navbar navbar-default">
    <div class="container-fluid">
      <div class="navbar-header">
        <form class="row g-3" action="DisplayStudents.php" method="POST">
          <div class="col-12">
        ...
        <br>
        <a href="DisplayStudents.php" class="btn btn-primary">Display all students</a>
      </div>
      <a href="../../StudentSpace/StudentSignup/StudentSignUp.php"><button class="btn btn-primary">Add a new Student</button></a>
      <a href="../DeleteHistory/DeleteStudentsHistory.php"><button class="btn btn-danger">Clear History</button></a>
    </div>
  </nav>
  <div class="container mt-3">
    <h2>Total of
      ...
      if (isset($_POST['Filter1'])) {
        ...
        $space = strpos($FilteredDiploma, ' ');
        $FilteredDiplomaCode = substr($FilteredDiploma, 0, $space);
        $q = "select count(IDS) from student where IDS in
              (select IDS from enroll where CourseCode in
              (select CourseCode from course where CodeD='$FilteredDiplomaCode'))";
        ...
        $q = "select count(IDS) from student where IDS in 
              (select IDS from enroll where CourseCode in 
                (select CourseCode from course where CourseCode='$FilteredCourseCode'))";
      } else $q = "select count(IDS) from student";
      $res = $PDO->prepare($q);
      $res->execute();
      $res = $res->fetch();
      $res = $res[0];
      if ($res <= 1) echo $res . " student";
      else echo $res . " students";
      ?>
    </h2>
      ...
      <table class="table table-striped table-bordered table-hover" style="border: 2px solid black">
        ...
        <tbody>
          ...
          if (isset($_POST['Filter1'])) {
            ...
            $space = strpos($FilteredDiploma, ' ');
            $FilteredDiplomaCode = substr($FilteredDiploma, 0, $space);
            $q = "select * from student where IDS in
                  (select IDS from enroll where CourseCode in
                  (select CourseCode from course where CodeD='$FilteredDiplomaCode'))";
          } else if (isset($_POST['Course'])) {
            $FilteredCourse = $_POST['Course'];
          ...
          while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
            ...
            $E = $row['Email'];
            echo "<td>
            <form method='POST' action='../Remove/RemoveStudent.php?Email=$E'>
            <input type='submit' value='Remove' class='btn btn-danger' onclick='return confirm(\"Are you sure you want to delete this student?\");'>
            </form>
            </td>";
