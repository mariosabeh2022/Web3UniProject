    ...
    <div class="container">
      ...
      <div class="collapse navbar-collapse" id="ftco-nav">
        <ul class="navbar-nav ml-auto">
          ...
          <li class="nav-item"><a href="../Export/DiplomaExportToExcel.php" class="nav-link">Export to excel sheet</a></li>
        </ul>
      </div>
    </div>
  </nav>
  <nav class="navbar navbar-default">
    <div class="container-fluid">
      <div class="navbar-header">
        <form class="row g-3" action="displayDiplomas.php" method="POST">
          <div class="col-12">
            ...
            <a href="DisplayDiplomas.php" class="btn btn-primary">Clear Filter</a>
          </div>
        </form>
      </div>
      <a href="../Add/AddDiploma.php"><button class="btn btn-primary">Add a new Diploma</button></a>
      <a href="../DeleteHistory/DeleteDiplomaHistory.php"><button class="btn btn-danger">Clear History</button></a>
    </div>
  </nav>
  <div class="container mt-3">
    <h2>Total of
      ...
      if (isset($_POST['Filter'])) {
        ...
        if (empty($FilteredDiploma))
          $q = "select count(*) from diploma";
        else {
          $q = "select count(*) from diploma where Name='$FilteredDiploma'";
        }
      } else $q = "select count(*) from diploma";
      $res = $PDO->prepare($q);
      $res->execute();
      $res = $res->fetch();
      $res = $res[0];
      if ($res <= 1) echo $res . " diploma";
      else echo $res . " diplomas"
      ?>
    </h2>
      ...
      <table class="table table-striped table-bordered table-hover" style="border: 2px solid black">
        ...
        <tbody>
          ...
          if (isset($_POST['Diploma'])) {
            $FilteredDiploma = $_POST['Diploma'];
            if ($FilteredDiploma == "All")
              $q = "select CodeD,Name from diploma";
            else {
              $q = "select CodeD,Name from diploma where Name='$FilteredDiploma'";
            }
          } else $q = "select CodeD,Name from diploma";
          $res = $PDO->prepare($q);
          $res->execute();
          while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
            echo "<tr class='coloredRow'>";

            echo "<td>" . $row['CodeD'] . "</td>";
            echo "<td>" . $row['Name'] . "</td>";
            $E = $row['CodeD'];
            echo "<td>
            <form method='POST' action='../Remove/RemoveDiploma.php?CodeD=$E'>
            <input type='submit' value='Remove' class='btn btn-danger' onclick='return confirm(\"Are you sure you want to delete this diploma?\");'>
            </form>
            </td>";
