...
if(!empty($_POST['FirstName'])&&
   ...
   !empty($_POST['Password'])){
    ...
    if($row=$statement->fetch(PDO::FETCH_ASSOC)){
        echo "<script>alert('You have entered an existing doctor  ');</script>";
    ...
    $query_user="INSERT INTO users (FirstName,MiddleName,LastName,Username,Email,Password,Role) VALUES
                ('$FN','$MN','$LN','$User','$Email','$Password','Doctor')";
    ...
    $query="INSERT INTO doctor (IDD,FirstName,MiddleName,LastName,Username,Email,Password,IDA)
...
header("refresh:3, url=DoctorSignUp.php");
?>